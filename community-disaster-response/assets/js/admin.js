...
function openGenericEdit(type, id, data) {
	...
	if (type === "volunteer") {
		...
		body.innerHTML =
			'<div class="form-group"><label>Full Name</label><input type="text" id="genEditFullName" value="' +
			(data.full_name || "").replace(/"/g, "&quot;") +
			'"></div>' +
			'<div class="form-group"><label>Skills</label><input type="text" id="genEditSkills" value="' +
			(data.skills || "").replace(/"/g, "&quot;") +
			'"></div>' +
			'<div class="form-group"><label>Availability</label><input type="text" id="genEditAvailability" value="' +
			(data.availability || "").replace(/"/g, "&quot;") +
			'"></div>';
		...
		sel += "</select>";
		body.innerHTML =
			'<div class="form-group"><label>Status</label>' + sel + "</div>";
	} else if (type === "help_request" || type === "donation") {
		title.textContent =
			type === "help_request"
				? "Edit Help Request Status"
...
function setupGenericEditModal() {
	...
	if (modal)
		modal.addEventListener("click", function (e) {
			if (e.target === modal) closeAndClear();
		});
				...
				try {
					...
					if (result.status === "success") {
						showAlert(
							...
							"Volunteer Updated",
							result.message ||
								"Volunteer information has been updated successfully.",
						);
						closeAndClear();
						loadVolunteers();
						loadDashboardStats();
						...
						showAlert(
							"error",
							"Update Failed",
							result.message || "Failed to update volunteer information.",
						);
						closeAndClear();
					}
				} catch (e) {
					showAlert(
						"error",
						"Error",
						"An error occurred while updating volunteer. Please try again.",
					);
					closeAndClear();
				}
			} else if (type === "emergency_report") {
				...
				try {
					...
					if (result.status === "success") {
						showAlert(
							...
							"Status Updated",
							result.message ||
								"Emergency report status has been updated successfully.",
						);
						closeAndClear();
						loadEmergencyReports();
						loadDashboardStats();
				...
				try {
					...
					if (result.status === "success") {
						showAlert(
							...
							"Status Updated",
							result.message ||
								"Help request status has been updated successfully.",
						);
						closeAndClear();
						loadHelpRequests();
						loadDashboardStats();
				...
				try {
					...
					if (result.status === "success") {
						showAlert(
							...
							"Status Updated",
							result.message ||
								"Donation status has been updated successfully.",
						);
						closeAndClear();
						loadDonations();
						loadDashboardStats();
		...
		if (result.status === "success" && result.data) {
			...
			var statCards = [
				...
				{ label: "Active Emergencies", value: stats.active_emergencies || 0 },
				{
					...
					value: stats.active_help_requests || 0,
				},
...
async function loadEmergencyReports() {
	...
	try {
		...
		if (Array.isArray(result.data) && result.data.length > 0) {
			...
			thead.innerHTML =
				"<tr><th>ID</th><th>Type</th><th>Location</th><th>Description</th><th>Status</th><th>Date</th><th>Actions</th></tr>";
			...
			result.data.forEach(function (report) {
				...
				if (
					report.assigned_volunteers &&
					report.assigned_volunteers.length > 0
				) {
					assignedVolunteersHtml =
						'<div style="margin-top: 0.5rem;"><strong>Assigned Volunteers (' +
						report.assigned_count +
						'):</strong><ul style="margin: 0.25rem 0; padding-left: 1.5rem;">';
					...
					assignedVolunteersHtml =
						'<div style="margin-top: 0.5rem; color: #999;">No volunteers assigned yet</div>';
				}
				...
				tr.innerHTML =
					...
					'<button class="button btn-edit report-edit" data-id="' +
					report.id +
					'" data-status="' +
					(report.status || "") +
					'">Edit</button> ' +
					(report.status === "Pending"
						? '<button class="button btn-approve" data-id="' +
							report.id +
							'" data-action="In Progress">In Progress</button> '
						: "") +
					(report.status !== "Done"
						? '<button class="button btn-done" data-id="' +
							report.id +
							'" data-type="EmergencyReport">Done</button> '
						: "<span>Completed</span> ") +
					'<button class="button btn-reject report-delete" data-id="' +
					report.id +
					'">Delete</button>' +
					"</div></td>";
			...
			container.querySelectorAll(".report-edit").forEach(function (btn) {
				btn.addEventListener("click", function () {
					openGenericEdit("emergency_report", btn.getAttribute("data-id"), {
	...
	try {
		...
		if (result.status === "success") {
			showAlert(
				...
				result.message || "Emergency report has been deleted successfully.",
			);
			loadEmergencyReports();
			loadDashboardStats();
		} else {
			showAlert(
				"error",
				"Deletion Failed",
				result.message || "Failed to delete report. Please try again.",
			);
		}
	} catch (e) {
		showAlert(
...
async function loadHelpRequests() {
	...
	try {
		...
		if (Array.isArray(result.data) && result.data.length > 0) {
			...
			var thead = document.createElement("thead");
			thead.innerHTML =
				"<tr><th>ID</th><th>Type</th><th>Description</th><th>Status</th><th>Date</th><th>Actions</th></tr>";
			table.appendChild(thead);
			var tbody = document.createElement("tbody");
			...
			result.data.forEach(function (request) {
				...
				tr.innerHTML =
					...
					"</td>" +
					'<td><div class="action-buttons">' +
					'<button class="button btn-edit help-edit" data-id="' +
					request.id +
					'" data-status="' +
					(request.status || "") +
					'">Edit</button> ' +
					(request.status === "Pending"
						...
						: "") +
					(request.status === "Pending"
						? '<button class="button btn-reject" data-id="' +
							request.id +
							'">Reject</button> '
						: "") +
					(request.status !== "Done"
						? '<button class="button btn-done" data-id="' +
							request.id +
							'" data-type="HelpRequest">Done</button> '
						: "<span>Completed</span> ") +
					'<button class="button btn-reject help-delete" data-id="' +
					request.id +
					'">Delete</button>' +
					"</div></td>";
				tbody.appendChild(tr);
			...
			container.querySelectorAll(".help-edit").forEach(function (btn) {
				btn.addEventListener("click", function () {
					openGenericEdit("help_request", btn.getAttribute("data-id"), {
						status: btn.getAttribute("data-status"),
					});
				});
			});
	...
	try {
		...
		if (result.status === "success") {
			showAlert(
				"success",
				"Status Updated",
				"Help request has been " + action + "d successfully.",
			);
			loadHelpRequests();
			loadDashboardStats();
		} else {
	...
	try {
		...
		if (result.status === "success") {
			showAlert(
				"success",
				"Successfully Deleted",
				result.message || "Help request has been deleted successfully.",
			);
			loadHelpRequests();
			loadDashboardStats();
		} else {
			showAlert(
				"error",
				"Deletion Failed",
				result.message || "Failed to delete help request. Please try again.",
			);
		}
	} catch (e) {
		showAlert(
...
async function loadVolunteers() {
	...
	try {
		...
		if (Array.isArray(result.data) && result.data.length > 0) {
			...
			var thead = document.createElement("thead");
			thead.innerHTML =
				"<tr><th>ID</th><th>Name</th><th>Skills</th><th>Availability</th><th>Status</th><th>Date</th><th>Actions</th></tr>";
			table.appendChild(thead);
			var tbody = document.createElement("tbody");
			...
			result.data.forEach(function (volunteer) {
				...
				tr.innerHTML =
					...
					"</td>" +
					'<td><div class="action-buttons">' +
					'<button class="button btn-edit volunteer-edit" data-id="' +
					volunteer.id +
					'" data-name="' +
					(volunteer.full_name || "").replace(/"/g, "&quot;") +
					'" data-skills="' +
					(volunteer.skills || "").replace(/"/g, "&quot;") +
					'" data-availability="' +
					(volunteer.availability || "").replace(/"/g, "&quot;") +
					'">Edit</button> ' +
					(isPending
						? '<button class="button btn-approve" data-id="' +
							volunteer.id +
					...
					(isPending
						? '<button class="button btn-reject" data-id="' +
							...
							'">Reject</button> '
						: "") +
					'<button class="button btn-reject volunteer-delete" data-id="' +
					volunteer.id +
					'">Delete</button>' +
					"</div></td>";
				tbody.appendChild(tr);
	...
	try {
		...
		if (result.status === "success") {
			showAlert(
				"success",
				"Status Updated",
				"Volunteer has been " + action + "d successfully.",
			);
			loadVolunteers();
			loadDashboardStats();
		} else {
		...
		showAlert(
			"error",
			"Error",
			"An error occurred while updating volunteer status. Please try again.",
		);
	}
}
	...
	try {
		...
		if (result.status === "success") {
			showAlert(
				"success",
				"Successfully Deleted",
				result.message || "Volunteer has been deleted successfully.",
			);
			loadVolunteers();
			loadDashboardStats();
		} else {
			showAlert(
				"error",
				"Deletion Failed",
				result.message || "Failed to delete volunteer. Please try again.",
			);
		}
	} catch (e) {
		showAlert(
...
async function loadDonations() {
	...
	try {
		...
		if (Array.isArray(result.data) && result.data.length > 0) {
			...
			var thead = document.createElement("thead");
			thead.innerHTML =
				"<tr><th>ID</th><th>Donor</th><th>Type</th><th>Quantity</th><th>Status</th><th>Date</th><th>Actions</th></tr>";
			table.appendChild(thead);
			var tbody = document.createElement("tbody");
			...
			result.data.forEach(function (donation) {
				...
				tr.innerHTML =
					...
					"</td>" +
					'<td><div class="action-buttons">' +
					'<button class="button btn-edit donation-edit" data-id="' +
					donation.id +
					'" data-status="' +
					(donation.status || "") +
					'">Edit</button> ' +
					...
					(isPending
						? '<button class="button btn-reject" data-id="' +
							...
							'">Reject</button> '
						: "") +
					'<button class="button btn-reject donation-delete" data-id="' +
					donation.id +
					'">Delete</button>' +
					"</div></td>";
				tbody.appendChild(tr);
			...
			container.querySelectorAll(".donation-edit").forEach(function (btn) {
				btn.addEventListener("click", function () {
					openGenericEdit("donation", btn.getAttribute("data-id"), {
						status: btn.getAttribute("data-status"),
					});
				});
			});
	...
	try {
		...
		if (result.status === "success") {
			showAlert(
				"success",
				"Status Updated",
				"Donation has been " + action + "d successfully.",
			);
			loadDonations();
			loadDashboardStats();
		} else {
			showAlert(
				"error",
				"Update Failed",
				result.message || "Failed to update donation status. Please try again.",
			);
		}
	} catch (error) {
		showAlert(
			"error",
			"Error",
			"An error occurred while updating donation status. Please try again.",
		);
	}
}
	...
	try {
		...
		if (result.status === "success") {
			showAlert(
				"success",
				"Successfully Deleted",
				result.message || "Donation has been deleted successfully.",
			);
			loadDonations();
			loadDashboardStats();
		} else {
			showAlert(
				"error",
				"Deletion Failed",
				result.message || "Failed to delete donation. Please try again.",
			);
		}
	} catch (e) {
		showAlert(
...
async function loadEvacuationCentersAdmin() {
	...
	try {
		...
		if (list.length === 0) {
			container.innerHTML =
				'<p>No evacuation centers. Click "Add Evacuation Center" to add one.</p>';
			return;
		}
		var table = document.createElement("table");
		table.innerHTML =
			"<thead><tr><th>Name</th><th>Address</th><th>Capacity</th><th>Status</th><th>Actions</th></tr></thead><tbody></tbody>";
		...
		list.forEach(function (row) {
			...
			tr.innerHTML =
				...
				"</td>" +
				'<td><div class="action-buttons">' +
				'<button class="button btn-edit evac-edit" data-id="' +
				row.id +
				'" data-name="' +
				(row.name || "").replace(/"/g, "&quot;") +
				'" data-address="' +
				...
				(row.status || "") +
				'">Edit</button> ' +
				'<button class="button btn-reject evac-delete" data-id="' +
				row.id +
				'">Delete</button>' +
				"</div></td>";
			tbody.appendChild(tr);
...
function openEvacCenterModal(data) {
	...
	document.getElementById("evacCenterId").value =
		data && data.id ? data.id : "";
	document.getElementById("evacCenterName").value =
		data && data.name ? data.name : "";
	document.getElementById("evacCenterAddress").value =
		data && data.address ? data.address : "";
	document.getElementById("evacCenterCapacity").value =
		data && data.capacity !== undefined ? data.capacity : "0";
	document.getElementById("evacCenterStatus").value =
		data && data.status ? data.status : "Available";
	title.textContent =
		data && data.id ? "Edit Evacuation Center" : "Add Evacuation Center";
...
function setupEvacuationCentersModal() {
	...
	if (form) {
		form.addEventListener("submit", async function (e) {
			...
			var capacity =
				parseInt(document.getElementById("evacCenterCapacity").value, 10) || 0;
			...
			try {
				...
				if (result.status === "success") {
					showAlert(
						"success",
						"Successfully Saved",
						result.message || "Evacuation center has been saved successfully.",
					);
					closeAndReset();
					loadEvacuationCentersAdmin();
				} else {
					showAlert(
						...
						"Save Failed",
						result.message ||
							"Failed to save evacuation center. Please try again.",
					);
					form.reset();
				}
			} catch (err) {
	...
	try {
		...
		if (result.status === "success") {
			showAlert(
				"success",
				"Successfully Deleted",
				result.message || "Evacuation center has been removed successfully.",
			);
			loadEvacuationCentersAdmin();
		} else {
			showAlert(
				...
				"Deletion Failed",
				result.message ||
					"Failed to delete evacuation center. Please try again.",
			);
		}
	} catch (error) {
		showAlert(
...
async function loadGuidesAdmin() {
	...
	try {
		...
		var table = document.createElement("table");
		table.innerHTML =
			"<thead><tr><th>Title</th><th>Category</th><th>Content</th><th>Archived</th><th>Actions</th></tr></thead><tbody></tbody>";
		var tbody = table.querySelector("tbody");
		list.forEach(function (row) {
			...
			tr.innerHTML =
				...
				"</td>" +
				'<td><div class="action-buttons">' +
				'<button class="button btn-edit guide-edit" data-id="' +
				row.id +
				'" data-title="' +
				(row.title || "").replace(/"/g, "&quot;") +
				'" data-content="' +
				...
				(row.category || "Other") +
				'">Edit</button> ' +
				'<button class="button btn-reject guide-archive" data-id="' +
				row.id +
				'">Archive</button> ' +
				'<button class="button btn-reject guide-delete" data-id="' +
				row.id +
				'">Delete</button>' +
				"</div></td>";
			tbody.appendChild(tr);
...
function openGuideModal(data) {
	...
	document.getElementById("guideModalTitle").textContent =
		data && data.id ? "Edit Guide" : "Add Guide";
	if (modal) modal.style.display = "flex";
}
	...
	if (form) {
		form.addEventListener("submit", async function (e) {
			...
			var payload = id
				...
				: { action: "add", title: title, content: content, category: category };
			try {
				var res = await fetch("../api/admin/preparedness_guides.php", {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify(payload),
				});
				...
				if (result.status === "success") {
					showAlert(
						"success",
						"Successfully Saved",
						result.message || "Guide has been saved successfully.",
					);
					...
					showAlert(
						"error",
						"Save Failed",
						result.message || "Failed to save guide. Please try again.",
					);
				}
				form.reset();
			} catch (err) {
...
function confirmArchiveGuide(id) {
	if (typeof swal === "function") {
		swal({
			title: "Archive Guide?",
			text: "It will be hidden from the public.",
			icon: "warning",
			buttons: true,
			dangerMode: true,
		}).then(function (will) {
...
async function archiveGuide(id) {
	try {
		var res = await fetch("../api/admin/preparedness_guides.php", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ action: "archive", id: parseInt(id, 10) }),
		});
		var result = await res.json();
		if (result.status === "success") {
			showAlert(
				...
				result.message || "Guide has been archived successfully.",
			);
			loadGuidesAdmin();
		} else {
			showAlert(
				"error",
				"Archive Failed",
				result.message || "Failed to archive guide. Please try again.",
			);
		}
	} catch (e) {
		showAlert(
...
function confirmDeleteGuide(id) {
	if (typeof swal === "function") {
		swal({
			title: "Delete Guide?",
			text: "This will permanently remove the guide.",
			icon: "warning",
			buttons: true,
			dangerMode: true,
		}).then(function (will) {
	...
	try {
		...
		if (result.status === "success") {
			showAlert(
				"success",
				"Successfully Deleted",
				result.message || "Guide has been deleted successfully.",
			);
			loadGuidesAdmin();
		} else {
			showAlert(
				"error",
				"Deletion Failed",
				result.message || "Failed to delete guide. Please try again.",
			);
		}
	} catch (e) {
		showAlert(
	...
	try {
		...
		if (result.status === "success") {
			showAlert(
				"success",
				"Marked as Done",
				"Request has been marked as done successfully. Volunteer totals have been updated.",
			);
			...
			showAlert(
				"error",
				"Update Failed",
				result.message || "Failed to mark request as done. Please try again.",
			);
		}
	} catch (error) {
		showAlert(
			"error",
			"Error",
			"An error occurred while marking request as done. Please try again.",
		);
	}
}
